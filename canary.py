...
# cargo update shiguredo_svt_av1 を実行
def run_cargo_update(dry_run: bool) -> None:
    if dry_run:
        print("Dry-run: Would run 'cargo update shiguredo_svt_av1'")
    else:
        subprocess.run(["cargo", "update", "shiguredo_svt_av1"], check=True)
        print("cargo update shiguredo_svt_av1 executed")
